...
int dvdLayout::configure()
{
	...
	for( i=0; i < Lfiles->size(); i++ )
	{
		...
		lFile->details = _f( "%s:%s :%7d :%7d :%5d%s:%7d%s:%7d :%6d : %s :%2d",
			...
			audioFrames,
			lFile->videoFrames,
			timestamp( titleVframes, job->tv == NTSC ),
			lFile->jpgIndex );
	...
	for( i=0; i < jpegs.size(); i++ )
	{
		LOG( _f( "%2d (%s%s%s %s) : %s\n", i,
			...
			jpegs[i].aspStr(),
			jpegs[i].fName.GetFullPath().c_str() ) );
	}
	ECHO( "\n\n" );
...
int dvdLayout::getNext()
{
	...
	if( writeFile->type & lpcmFile::seqStart )
		nameNow = _f( "%s%s_title_%02d-%02d", job->tempPath.GetFullPath().c_str(),
...
//      SCRN( INFO_TAG << TINT( _f( "Title %d - (%s / %s %s)\n\n",
		SCRN( "\n" /*<< INFO_TAG*/ << TINT( _f( "Title %d - (%s / %s %s)\n\n",
			writeFile->group + 1, lpcmEntity::audioInfo( writeFile ),
...
const char* dvdauthorXml::timestampFractional( uint32_t f, bool ntsc, float boost )
{
	uint16_t hour, min;
	uint64_t ticks = f * ( ntsc ? 3003 : 3600 );
	double sec = (double)ticks / 90000;
...
//            xml << _f( "<dvdstyler name=\"%s\" videoFormat=\"%d\" aspectRatio=\"1\" audioFormat=\"%d\" videoBitrate=\"-1\" capacity=\"0\" emptyMenu=\"1\" format=\"4\" jumppad=\"1\">\n\n",
				xml << _f( "<dvdstyler name=\"%s\" videoFormat=\"%d\" aspectRatio=\"%d\">\n\n",
					wxFileName( str ).GetName().c_str(), video == NTSC ? 3 : 2, flag ? 2 : 1 );
#endif
