...
/* static  */const char * ifoPrint_audio_attributes( audio_attr_t *attr )
{
	wxString info;
		...
		while( ++audioStream < numAudioStreams )
		{
			if( verbose )
				INFOv( _f( "Title %02d audio stream %d: %s\n", titleset, audioStream,
					ifoPrint_audio_attributes( &ifo->vtsi_mat->vts_audio_attr[audioStream] ) ) );
			...
			look - buf + 16 < ( 2 * DVD_VIDEO_LB_LEN ); look++ )
		{
			if( look[0] == '\0' )
				if( bEndian32( look ) == 0x000001B8 )
					...
					{
						ECHO( _f( "%s(%s) Lplex tags found in GOP User Data field at LB %d::0x%03x\n",
...
void lpcmPGextractor::udfCopyInfoFiles( const char * path )
{
	...
	outPath = ( path ? (char*)path : job->extractPath.GetFullPath() );
	...
	for( int i=0; i < infofiles->size(); i++ )
	{
		...
		iName = iFile->fName;
		_wxFixSeparators( (char*)iName.c_str() );
		INFO( "-copying '" << iName << "'\n" );
